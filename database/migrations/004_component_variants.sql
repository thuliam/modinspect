...
((SELECT id FROM product_categories WHERE slug='motherboard'),(SELECT id FROM brands WHERE slug='asus'),'Prime A620M-K','asus-prime-a620m-k','ASUS Prime A620M-K','AM5 entry-level, 2 DIMM, M.2 PCIe 4.0','AM5','A620','DDR5','Micro-ATX','assets/images/admin-5.jpg',1),
((SELECT id FROM product_categories WHERE slug='motherboard'),(SELECT id FROM brands WHERE slug='msi'),'MAG B650 Tomahawk WiFi','msi-mag-b650-tomahawk-wifi','MSI MAG B650 Tomahawk WiFi','AM5, Wi-Fi 6E, 3x M.2, 2.5G LAN','AM5','B650','DDR5','ATX','assets/images/admin-5.jpg',1),
((SELECT id FROM product_categories WHERE slug='motherboard'),(SELECT id FROM brands WHERE slug='gigabyte'),'X670 AORUS Elite AX','gigabyte-x670-aorus-elite-ax','Gigabyte X670 AORUS Elite AX','AM5, PCIe 5.0 M.2, Wi-Fi 6E','AM5','X670','DDR5','ATX','assets/images/admin-5.jpg',1),
((SELECT id FROM product_categories WHERE slug='motherboard'),(SELECT id FROM brands WHERE slug='asus'),'TUF Gaming Z790-PLUS WiFi D4','asus-tuf-z790-plus-wifi-d4','ASUS TUF Gaming Z790-PLUS WiFi D4','LGA1700, Wi-Fi 6, PCIe 5.0','LGA1700','Z790','DDR4','ATX','assets/images/admin-5.jpg',1);
...
FROM products p JOIN (
 ...
 SELECT 'intel-core-i5-13600k',7300,7900,8500,9200,9900 UNION ALL
 SELECT 'asus-prime-a620m-k',2100,2400,2700,3000,3400 UNION ALL
 SELECT 'msi-mag-b650-tomahawk-wifi',5200,5700,6200,6800,7500 UNION ALL
 SELECT 'gigabyte-x670-aorus-elite-ax',6500,7100,7800,8500,9300 UNION ALL
 SELECT 'asus-tuf-z790-plus-wifi-d4',5900,6500,7100,7800,8500
) v ON v.slug=p.slug
WHERE NOT EXISTS (SELECT 1 FROM price_indices pi WHERE pi.product_id=p.id);

INSERT IGNORE INTO product_variants (product_id,brand_id,variant_name,slug,cooler_type,is_premium,notes)
SELECT id,brand_id,CONCAT(model_name,' Tray'),'tray',NULL,0,'ไม่มีชุดระบายความร้อนและกล่องแบบ Retail' FROM products WHERE category_id=(SELECT id FROM product_categories WHERE slug='cpu');
INSERT IGNORE INTO product_variants (product_id,brand_id,variant_name,slug,cooler_type,is_premium,notes)
SELECT id,brand_id,CONCAT(model_name,' Box'),'box','Stock cooler',1,'กล่อง Retail; บางรุ่นมีชุดระบายความร้อน' FROM products WHERE category_id=(SELECT id FROM product_categories WHERE slug='cpu');
